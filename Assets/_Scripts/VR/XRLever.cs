...
namespace UnityEngine.XR.Content.Interaction
{
    public class XRLever : XRBaseInteractable
    {
        ...
        [SerializeField]
        protected float m_ValueToClamp = 1.0f;
        ...
        protected UnityEvent m_OnLeverActivate = new();

        [SerializeField]
        [Tooltip("Events to trigger when the lever deactivates")]
        protected UnityEvent m_OnLeverDeactivate = new();
        [SerializeField]
        protected UnityEvent<float> m_OnLeverValueChanged = new();
        ...
        {
            if(Mathf.Abs(_currentAngle) < m_ValueToClamp) _currentAngle = 0;

            SetValue(m_Value, m_LockToValue);
            m_Interactor = null;
        }
        ...
        void SetHandleAngle(float angle)
        {
            ...
            m_OnLeverValueChanged.Invoke(GetNormalizedValue());
        }
        
        
        public float GetNormalizedValue()
        {
            float norm = Mathf.InverseLerp(m_MinAngle, m_MaxAngle, _currentAngle) * 2f - 1f;
